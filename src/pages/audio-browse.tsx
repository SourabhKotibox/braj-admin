...
import { useGetPublicAudios, getImageUrl } from "@/lib/api-client";
...
export default function AudioBrowsePage() {
  ...
  const { data, isLoading } = useGetPublicAudios({ search });
  const audios = (data?.data || []).map((a: any) => ({
    ...a,
    id: a.id || a._id,
    audioUrl: a.audioUrl || a.audioQualities?.[0]?.url || "",
  }));

  const playUrl = selectedAudio
    ? (selectedAudio.audioUrl?.startsWith("http") ? selectedAudio.audioUrl : getImageUrl(selectedAudio.audioUrl))
    : "";
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          {selectedAudio ? (
            <div className="max-w-3xl mx-auto space-y-6">
              <button onClick={() => setSelectedAudio(null)} className="text-white/70 hover:text-white">← Back to Audio</button>
              <AudioPlayer
                src={playUrl}
                title={selectedAudio.title}
                artist={selectedAudio.artist}
                thumbnail={getImageUrl(selectedAudio.thumbnail || selectedAudio.coverImage || "")}
              />
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setLocation("/music")} className="text-sm text-zinc-400 hover:text-white">Open music player →</button>
              </div>
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {audios.map((audio: any) => (
                    <div key={audio.id} className="group cursor-pointer" onClick={() => setSelectedAudio(audio)}>
                      <div className="relative aspect-square rounded-xl overflow-hidden bg-zinc-800 mb-2">
                        {audio.thumbnail || audio.coverImage ? (
                          <img src={getImageUrl(audio.thumbnail || audio.coverImage)} alt={audio.title} className="w-full h-full object-cover" />
                        ) : (
