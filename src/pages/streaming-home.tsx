...
const MUSIC_DROPDOWN = [
  { label: "Music Audio", href: "/music", icon: <Headphones className="w-4 h-4" />, description: "Listen to devotional songs" },
  { label: "Music Videos", href: "/videos", icon: <Video className="w-4 h-4" />, description: "Watch music videos" },
];

export function PublicHeader({ activeTab, setActiveTab, onSignIn, onSignOut, user, onSubscribeClick, onNavigate }: {
  activeTab: Tab; setActiveTab: (t: Tab) => void; onSignIn: () => void; onSignOut?: () => void; user?: any; onSubscribeClick?: () => void; onNavigate?: (path: string) => void;
}) {
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-[#0a0a10]/95 backdrop-blur-md shadow-[0_2px_24px_rgba(0,0,0,0.85)] border-b border-white/5" : "bg-gradient-to-b from-[#030306]/95 via-[#030306]/40 to-transparent"}`}>
              ...
              <nav className="hidden lg:flex items-center gap-1">
                ...
                <div className="relative" ref={musicRef}>
                  ...
                  >
                    ...
                    Music
                    <ChevronDown className={`w-3 h-3 transition-transform ${musicDropdownOpen ? "rotate-180" : ""}`} />
                  </button>
                  {musicDropdownOpen && (
                    <div className="absolute top-full left-0 mt-2 w-56 bg-[#0a0a0a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
                      {MUSIC_DROPDOWN.map((item) => (
                        <button
                          key={item.href}
                          onClick={() => {
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-300 ${mobileOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="bg-[#0a0a10] border-t border-zinc-800 px-4 py-4 space-y-2">
            ...
            {/* Music Dropdown for Mobile */}
            <div className="pt-2 border-t border-white/5">
              <p className="px-3.5 py-2 text-xs font-bold text-white/50 uppercase tracking-wider">Music</p>
              {MUSIC_DROPDOWN.map((item) => (
                <button
                  key={item.href}
                  onClick={() => { setLocation(item.href); setMobileOpen(false); }}
                  className="w-full flex items-center gap-3 px-3.5 py-3 rounded-full text-sm font-bold transition-all text-white hover:bg-white/10"
                >
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2.5">
              {[
                ...
                { label: "Music Videos", href: "/videos" },
...
/* ─── MAIN PAGE ─── */
const VALID_TABS: Tab[] = ["home", "movies", "tvshows", "drama", "new"];
...
export default function StreamingHomePage() {
  ...
  useEffect(() => {
    ...
    const appName = settings?.platformName || "Braj Cinema TV";
